            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (typeof initCallback === 'function') {
                        ...
                        else if (url.includes('item_price_settings.html')) setTimeout(() => window.initPriceSettings && window.initPriceSettings(), 10);
                    }
                } else {
...
// --- CHART OF ACCOUNTS (COA) LOGIC ---
        ...
        let priceItemsData = [];
        window.initPriceSettings = async function() {
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            ...
            try {
                ...
                loadPriceSettingsData();
            } catch (e) { 
                console.error("Price Settings Init Error:", e);
                const tbody = document.getElementById('priceSettingsTableBody');
                if (tbody) tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px; color: red;">Error initializing form. Check console.</td></tr>';
            }
        };
                    ...
                    <tr data-id="${item.id}">
                        ...
                        <td>
                            <input type="number" step="0.01" class="price-input-cell" 
                                   ...
                                   onchange="updateLocalPrice(${item.id}, this.value)">
                        </td>
                    </tr>
                `;
            });
        ...
        window.updateLocalPrice = function(id, val) {
            const item = priceItemsData.find(i => i.id === id);
        ...
        window.saveBulkPrices = async function() {
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            const companyId = session.company_id || 1;
            const payload = { prices: priceItemsData.map(i => ({ item_id: i.id, selling_price: i.selling_price })) };
