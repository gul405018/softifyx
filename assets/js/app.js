            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (typeof initCallback === 'function') {
                        ...
                        else if (url.includes('inventory_coa.html')) setTimeout(() => initInventoryCOAView(), 10);
                    }
                } else {
        ...
        if(navRes.ok) {
            ...
            document.querySelectorAll('.dropdown-item[data-target], .nested-item[data-target]').forEach(item => {
                item.addEventListener('click', (e) => {
                    ...
                    let isInv = (moduleName === "Chart Of Inventory" || (targetUrl && targetUrl.includes('inventory_coa.html')));
...
// --- CHART OF ACCOUNTS (COA) LOGIC ---
        ...
        async function initInventoryCOAView() {
        ...
        async function loadInvMain() {
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            const coId = session.company_id || 1;
            try {
                const res = await fetch(`api/inventory.php?action=get_main_categories&company_id=${coId}`);
                if (res.ok) {
                    invMainPool = await res.json();
                    renderInvMainList();
                    resetInvMainForm();
                }
            } catch (err) {}
        ...
        function renderInvMainList() {
            ...
            if (!list) return;
            list.innerHTML = invMainPool.map(m => `<option value="${m.id}">${m.code} - ${m.name}</option>`).join('');
        }

        async function onInvMainSelect(id) {
            selectedInvMainId = id;
            const main = invMainPool.find(m => m.id == id);
            if (main) {
                document.getElementById('invMainCode').value = main.code;
                document.getElementById('invMainName').value = main.name;
                document.getElementById('invMainName').disabled = false;
                
                // Clear Sub and Item forms
                resetInvSubForm();
                resetInvItemForm();
        ...
        async function loadInvSub(mainId) {
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            const coId = session.company_id || 1;
            try {
                const res = await fetch(`api/inventory.php?action=get_sub_categories&company_id=${coId}&main_id=${mainId}`);
                if (res.ok) {
                    invSubPool = await res.json();
                    renderInvSubList();
                }
            } catch (err) {}
        }
        ...
        function renderInvSubList() {
            ...
            if (!list) return;
            list.innerHTML = invSubPool.map(s => `<option value="${s.id}">${s.code} - ${s.name}</option>`).join('');
        }

        async function onInvSubSelect(id) {
            selectedInvSubId = id;
            const sub = invSubPool.find(s => s.id == id);
            if (sub) {
                document.getElementById('invSubCode').value = sub.code;
                document.getElementById('invSubName').value = sub.name;
                document.getElementById('invSubName').disabled = false;
                
                // Reset Item
                resetInvItemForm();
                
                // Load Items
                await loadInvItems(id);
            }
        }

        async function loadInvItems(subId) {
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            const coId = session.company_id || 1;
            try {
                const res = await fetch(`api/inventory.php?action=get_items&company_id=${coId}&sub_id=${subId}`);
                if (res.ok) {
                    invListPool = await res.json();
                    renderInvItemList();
                }
            } catch (err) {}
        }
        ...
        function renderInvItemList() {
            ...
            if (!list) return;
            list.innerHTML = invListPool.map(i => `<option value="${i.id}">${i.code} - ${i.name}</option>`).join('');
        }

        function onInvItemSelect(id) {
            selectedInvItemId = id;
            const item = invListPool.find(i => i.id == id);
            if (item) {
                ...
                document.getElementById('invItemBrand').value = item.brand_name || '';
                ...
                document.getElementById('invItemQtyPerPiece').value = item.qty_per_piece || 0;
                document.getElementById('invItemTaxRate').value = item.sales_tax_rate || 0;
                
                const taxRadios = document.getElementsByName('taxType');
                taxRadios.forEach(r => r.checked = (r.value === item.sales_tax_type));
                ...
                document.getElementById('invItemInactive').checked = parseInt(item.is_inactive) === 1;

                enableInvItemFields(true);
            }
        }

        function resetInvMainForm(isAdd = false) {
            selectedInvMainId = isAdd ? null : selectedInvMainId;
            const nameInp = document.getElementById('invMainName');
            const codeInp = document.getElementById('invMainCode');
            if (!nameInp) return;
            ...
            if (isAdd) {
                ...
                nameInp.focus();
                // MS-01 Auto Code
                const nextNo = invMainPool.length + 1;
                codeInp.value = 'MS-' + String(nextNo).padStart(2, '0');
            } else {
                nameInp.value = '';
                codeInp.value = '';
        ...
        function resetInvSubForm(isAdd = false) {
            if (isAdd && !selectedInvMainId) {
                alert("Please select a Main Category first.");
                return;
            }
            selectedInvSubId = isAdd ? null : selectedInvSubId;
            const nameInp = document.getElementById('invSubName');
            const codeInp = document.getElementById('invSubCode');
            if (!nameInp) return;
            ...
            if (isAdd) {
                ...
                nameInp.focus();
                // MS-0101 Logic
                const main = invMainPool.find(m => m.id == selectedInvMainId);
                const nextNo = invSubPool.length + 1;
                codeInp.value = main.code + String(nextNo).padStart(2, '0');
            } else {
                nameInp.value = '';
                codeInp.value = '';
                nameInp.disabled = true;
        ...
        function resetInvItemForm(isAdd = false) {
            if (isAdd && !selectedInvSubId) {
                alert("Please select a Sub Category first.");
                return;
            }
            selectedInvItemId = isAdd ? null : selectedInvItemId;
            enableInvItemFields(isAdd);
            
            if (isAdd) {
                document.getElementById('invItemName').value = '';
                document.getElementById('invItemDesc').value = '';
                document.getElementById('invItemRack').value = '';
                document.getElementById('invItemPurchasePrice').value = 0;
                document.getElementById('invItemSellingPrice').value = 0;
                document.getElementById('invItemUnit').value = 'Pcs';
                document.getElementById('invItemQtyPerPiece').value = 0;
                document.getElementById('invItemTaxRate').value = 0;
                document.getElementById('invItemValuationCost').value = 0;
                document.getElementById('invItemOrderQty').value = 0;
                document.getElementById('invItemInactive').checked = false;
                document.getElementById('invItemName').focus();

                // MS-01010001
                const sub = invSubPool.find(s => s.id == selectedInvSubId);
                const nextNo = invListPool.length + 1;
                document.getElementById('invItemCode').value = sub.code + String(nextNo).padStart(4, '0');
            } else {
        ...
        function enableInvItemFields(enabled) {
            ...
            const itemInputs = [
                ...
                'invItemPurchasePrice', 'invItemSellingPrice', 'invItemUnit',
                'invItemQtyPerPiece', 'invItemTaxRate', 'invItemValuationCost',
                'invItemOrderQty', 'invItemInactive'
            ];
        ...
        async function saveInvMain() {
            const name = document.getElementById('invMainName').value;
            const code = document.getElementById('invMainCode').value;
            ...
            try {
                const res = await fetch(`api/inventory.php?action=save_main_category&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify({ id: selectedInvMainId, name, code })
                });
                if (res.ok) {
                    await loadInvMain();
                }
        ...
        async function saveInvSub() {
            const name = document.getElementById('invSubName').value;
            const code = document.getElementById('invSubCode').value;
            ...
            try {
                const res = await fetch(`api/inventory.php?action=save_sub_category&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify({ id: selectedInvSubId, main_id: selectedInvMainId, name, code })
                });
                if (res.ok) {
                    await loadInvSub(selectedInvMainId);
                    resetInvSubForm();
                }
            } catch (err) {}
        }
        ...
        async function saveInvItem() {
            ...
            const code = document.getElementById('invItemCode').value;
            ...
            const taxRadios = document.getElementsByName('taxType');
            let taxType = 'Percent';
            taxRadios.forEach(r => { if(r.checked) taxType = r.value; });

            const valRadios = document.getElementsByName('costValuation');
            let valuationMethod = 'Weighted Average';
            valRadios.forEach(r => { if(r.checked) valuationMethod = r.value; });

            const payload = {
                id: selectedInvItemId,
                sub_id: selectedInvSubId,
                code: code,
                name: name,
                description: document.getElementById('invItemDesc').value,
                brand_name: document.getElementById('invItemBrand').value,
                ...
                sales_tax_rate: document.getElementById('invItemTaxRate').value,
            ...
            try {
                const res = await fetch(`api/inventory.php?action=save_item&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify(payload)
                });
                if (res.ok) {
                    await loadInvItems(selectedInvSubId);
                    resetInvItemForm();
                }
            } catch (err) {}
        }
        ...
        async function deleteInvMain() {
            if (!selectedInvMainId) return;
            if (confirm("Delete this category?")) {
                const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
                const coId = session.company_id || 1;
                const res = await fetch(`api/inventory.php?action=delete_main_category&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify({ id: selectedInvMainId })
                });
                if (res.ok) loadInvMain();
                else {
                    const data = await res.json();
        ...
        async function deleteInvSub() {
            if (!selectedInvSubId) return;
            if (confirm("Delete this sub-category?")) {
                const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
                const coId = session.company_id || 1;
                const res = await fetch(`api/inventory.php?action=delete_sub_category&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify({ id: selectedInvSubId })
                });
                if (res.ok) loadInvSub(selectedInvMainId);
                else {
                    const data = await res.json();
                    alert(data.error || "Delete failed");
                }
        ...
        async function deleteInvItem() {
            ...
            if (confirm("Delete this inventory item?")) {
                const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
                const coId = session.company_id || 1;
                const res = await fetch(`api/inventory.php?action=delete_item&company_id=${coId}`, {
                    method: 'POST',
                    body: JSON.stringify({ id: selectedInvItemId })
                });
                if (res.ok) loadInvItems(selectedInvSubId);
            }
        }

        async function loadInvBrands() {
            const brandSel = document.getElementById('invItemBrand');
            if(!brandSel) return;
            const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
            const coId = session.company_id || 1;
            try {
                ...
                if (res.ok) {
                    ...
                    brandSel.innerHTML = '<option value="">None</option>' + brands.map(b => `<option value="${b.name}">${b.name}</option>`).join('');
                }
            } catch (err) {}
        }
        ...
        async function manageBrands() {
            ...
            if (name) {
                const session = JSON.parse(localStorage.getItem('softifyx_session') || '{}');
                const coId = session.company_id || 1;
                await fetch(`api/inventory.php?action=save_brand&company_id=${coId}`, {
                    method: 'POST',
        ...
        // Global exposing
        window.initInventoryCOAView = initInventoryCOAView;
        ...
        window.printInvChart = (type) => alert("Printing " + type + " inventory chart...");
        window.printInvPriceList = (type) => alert("Printing " + type + " price list...");
